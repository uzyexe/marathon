...
import mesosphere.marathon.raml.{ App, ContainerPortMapping, DockerContainer, DockerNetwork, DockerPortProtocol, EngineType, EnvVarValueOrSecret, IpAddress, IpDiscovery, IpDiscoveryPort, Network, NetworkMode, NetworkProtocol, Raml, SecretDef }
...
class AppsResourceTest extends MarathonSpec with MarathonActorSupport with Matchers with Mockito with GivenWhenThen {
  ...
  def prepareApp(
    ...
    val group = Group(PathId("/"), Map(appDef.id -> appDef))
    ...
    val app = App(
      ...
      container = Some(raml.Container(
        ...
        docker = Some(DockerContainer(
          portMappings = Seq(ContainerPortMapping(containerPort = 0, protocol = Some(DockerPortProtocol.Tcp))),
          image = "jdef/helpme",
  ...
  test("Create a new app in BRIDGE mode w/ Docker") {
    ...
    val container = DockerContainer(
      ...
      image = "jdef/helpme",
      portMappings = Seq(
        ContainerPortMapping(containerPort = 0, protocol = Some(DockerPortProtocol.Tcp))
      )
    )
    ...
    val app = App(
      ...
      ipAddress = Some(IpAddress(
        ...
        discovery = Some(IpDiscovery(ports = Seq(
          IpDiscoveryPort(number = Some(1), name = Some("bob"), protocol = Some(NetworkProtocol.Tcp))
        )))
      )),
      container = Some(raml.Container(
        `type` = EngineType.Docker,
